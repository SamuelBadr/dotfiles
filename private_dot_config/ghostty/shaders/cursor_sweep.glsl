...
void mainImage(out vec4 fragColor, in vec2 fragCoord) {
    ...
    // Only render trail if cursor moved significantly, has valid dimensions, and animation is active
    if (lineLength > minDist && currentCursor.z > 0.001 && currentCursor.w > 0.001 && progress < 0.99) {
        ...
        // Punch hole for current cursor position
