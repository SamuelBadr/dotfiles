...
- For every agent-initiated deletion on macOS or Linux, never use `rm` or another permanent deletion command. Use `trash <paths...>` instead. macOS 15+ provides a native `trash` command; Linux and older macOS versions must provide the same interface with `trash-cli`.
