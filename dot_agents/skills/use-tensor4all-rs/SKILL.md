...
```toml
[dependencies]
# Reproducible — pin to a release tag:
tensor4all-simplett = { git = "https://github.com/tensor4all/tensor4all-rs", tag = "v0.2.0", package = "tensor4all-simplett" }
# Latest — float a rev instead of a tag:
...
- **Add only the entry crate you need** (step 2). It pulls its transitive `tensor4all-*` deps; do not list them yourself.
- **Backend defaults to pure-Rust `faer` — no system BLAS.** Every compute crate ships `default = ["tenferro-cpu-faer"]`, so a plain dependency compiles standalone. To link a system BLAS (OpenBLAS / MKL / Apple Accelerate) instead, set `default-features = false` and enable `tenferro-system-blas` on the entry crate where it is exposed.
...
Stack direction (dependencies flow downward):

```text
tensorbackend (internal)  →  tcicore  →  core  →  { treetn, itensorlike, simplett }
                                       simplett + tcicore → tensorci → partitionedtt
                                       treetn → treetci
                                       treetn + simplett → quanticstransform
                                       { treetci, quanticstransform, simplett, treetn } → quanticstci
hdf5 depends on core + itensorlike; capi depends on most crates.
```
...
- **`docs/api/*.md`** — generated by `cargo run -p api-dump --release -- . -o docs/api`. Read these before source files; they list the real public surface. Never hand-edit them.
...
**Dense layout is column-major (Fortran order).** `TensorDynLen::from_dense(indices, data)` and all flat buffers, `reshape`, the C API, and HDF5 use column-major: the **last listed index varies fastest**. Matches Julia / ITensors.jl. When feeding NumPy data, use `order="F"`.
...
**Tolerance is `rtol`, not `cutoff`.** SVD truncation uses relative tolerance `rtol`; ITensors.jl uses `cutoff`. Convert with `rtol = sqrt(cutoff)` (so `cutoff=1e-10` → `rtol=1e-5`). Singular values are indexed `s[[0, i]]`, not `s[[i, i]]`.

**Sweep counts differ by crate.** `tensor4all-treetn` and `quanticstransform` use `nfullsweeps` (each edge visited twice, forward+back). `tensor4all-itensorlike` uses `nhalfsweeps`. Relation: `nfullsweeps = nhalfsweeps / 2`. `ContractOptions`/`LinsolveOptions` provide `with_nsweeps(n)` = `with_nhalfsweeps(2 * n)`.
...
- Developer entry point: `AGENTS.md`. Durable repo rules: `REPOSITORY_RULES.md`. Shared agent rules: the `tensor4all-agent-rules` repo.
- Julia bindings: https://github.com/tensor4all/Tensor4all.jl (wraps `tensor4all-capi`).
- Design docs: `docs/design/index.md`. Plans: `docs/plans/`. C API design: `docs/CAPI_DESIGN.md`. Provenance/citation: `docs/PROVENANCE_AND_CITATION_POLICY.md`.
