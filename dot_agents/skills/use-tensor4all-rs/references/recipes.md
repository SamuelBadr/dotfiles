...
Runnable task patterns, distilled from the mdBook guides (`docs/book/src/`). Each is the
minimal correct shape — confirm option defaults against `docs/api/` before production use.
All examples use `f64`; swap in `Complex64` (from `num_complex`) for complex work via the
same generic entry points.

## Dependency

Declare one `tensor4all-*` entry crate as a git/path dependency and build in
release — see [`SKILL.md`](SKILL.md) §1 for the authoritative setup (tag
pinning, backend features, `--release`, dev-deps). Each recipe below assumes that
is done.
...
// save_mps("state.h5", &tt, "MPS")?;      // or save_itensor for a single ITensor
// let loaded = load_mps::<TensorTrain>("state.h5", "MPS")?;
```
